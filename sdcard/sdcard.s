...
;****************************************
init_sdcard
	lda IO_1+PRB					; Detect card
	and #0x04						; Bit 2 of PRB = 0
	sta sd_status
	bne init_sd_failed				; If not zero then no card
...
sd_shiftskiplo
	...
	inc SD_REG						; Set bit 1 of clock
;	dec SD_REG						; Reset bit 1 of clock
	sta SD_REG						; Reset bit 1 of clock
...
sd_busy
	pha
	lda sd_status					; If no card then fall out
	bne sd_isbusy_fin
sd_isbusy
	jsr sd_getbyte
...
sd_getrespR17
	...
	ldy sd_status
	bne sd_getrespR17_fin

	sta tmp_v1+1					; Page to read in to
	stz tmp_v1						; Always a page boundary
sd_getrespR17token
...
sd_getrespR17block1
	jsr sd_getbyte					; get a byte
	sta (tmp_v1),y					; Save the byte
	iny								; Keep going
	bne sd_getrespR17block1			; Until all bytes read

	inc tmp_v1+1					; Next page
sd_getrespR17block2
...
sd_sendcmd24
	...
	sta tmp_v1+1
	stz tmp_v1					; Address is always page boundary
...
sd_writeblock_1					; Send first 256 bytes
	...
	iny
	bne sd_writeblock_1
	inc tmp_v1+1				; Next page for second 256 bytes
...
sd_msg_failed
	db "\rBad or missing SD Card\r", 0
