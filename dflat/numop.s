...
df_rt_sgte
	jsr df_rt_str_comp
	bcs df_rt_str_comp_true
	bcc df_rt_str_comp_false
...
df_rt_str_comp_byte
	lda (df_tmpptra),y
	cmp (df_tmpptrb),y
	bne df_rt_str_comp_done
	; if here then both strings still the same
	cmp #0				; What was op1 char
	; if char is zero then end
	beq df_rt_str_comp_done
	; else do next char
	iny
