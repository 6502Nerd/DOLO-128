...
;* kept about the VDP current mode
	...
	db vdp_gmode				;* Graphics mode 0,1,2 or 0x80 (hi-res)
	end struct
...
;* accessing the file system
	...
	ds FH_Attr, 1				; What kind of file ** keep straight after name **
	ds FH_Size, 4				; File size
	ds FH_CurrClust, 2			; Current cluster
	ds FH_SectCounter, 1		; Sector counter to know when next cluster is needed
	ds FH_CurrSec, 4			; Current sector
 	ds FH_Pointer, 4			; Pointer in to file for next byte
	ds FH_DirSect, 4			; Parent directory sector
	ds FH_DirOffset, 2			; Offset of this entry in to current directory sector
	ds FH_FirstClust, 2			; First cluster of file data
	ds FH_LastClust, 2			; Last cluster accessed
	ds FH_TimeDate, 5			; Time-Date created (ms, time, date)
	ds FH_FileMode, 1			; File mode (0=read, else write)
	ds FH_FSpecPtr, 2			; Pointer to file spec being searched, created, loaded
	end struct
