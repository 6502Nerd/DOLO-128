...
mon_ready
	...
	lda (df_linbuff),y			; Get the command byte (alpha)
...
cmd_jsr
	jmp (tmp_b)					; Jump to the command to execute - return with rts
...
mon_read_byte
	lda (df_linbuff),y			; Load high char
	pha
	iny							; Point to high char
	lda (df_linbuff),y			; Load low char
	tax							; X = Low
	pla							; A = High
...
mon_cmd_game
	...
	sta spr0_t
game_loop
	lda vdp_int_cnt			; vdp_int_cnt is updated every VB
	sta spr0_t
	jsr snd_get_joy0
	tya
	_printA
