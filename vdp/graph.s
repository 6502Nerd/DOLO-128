...
;****************************************
gr_init_hires
	sta gr_scrngeom+gr_pixcol		; Save pixel colour

	inc vdp_curoff					; Switch cursor off
...
;****************************************
gr_init_screen
	sta vdp_base+vdp_gmode
	cmp #0x10				; Hires mode?
	bcs gr_init_screen_hires
	cmp #0
	bne gr_init_skip_txt
	jmp gr_init_screen_txt
gr_init_skip_txt
	jmp gr_init_screen_32
...
;****************************************
gr_hchar
	stx tmp_b				; Save X coord
	sty tmp_b+1				; Save Y coord
	
	; Calculate font address of char code A in to tmp_c word
	sta tmp_c
	stz tmp_c+1
	; Multiply by 8
	asl tmp_c
...
gr_hchar_getimage
	...
	sta tmp_c			; Save in temp c
	plx					; Restore index but don't lose it
	phx
	lda ztmp_16,x		; Get cached image
	ldy gr_scrngeom+gr_pixmode		; Get the mode to plot 0,1,2 (erase, draw, xor)
	beq gr_hchar_getimage_plot		; If erase then blat over for text plotting
	eor tmp_c			; First assume EOR with vram
	cpy #2				; Is that the mode
	beq gr_hchar_getimage_plot
	ora tmp_c			; Else OR VRAM with Image
gr_hchar_getimage_plot
	; Get vram address from tmp_v1
	ldx tmp_v1
	ldy tmp_v1+1
	jsr vdp_poke		; Put image on

	ldy gr_scrngeom+gr_pixmode
	cpy #2
	beq gr_hchar_skipcol
...
gr_hchar_skipcol
	inc tmp_b+1			; Update y coord
	plx					; Restore index
	inx
	cpx #8				; Done all of left side?
	beq gr_hchar_prepright
	cpx #16				; We've done all of right side?
	beq gr_hchar_imagedone
	bne gr_hchar_getimage	; Else keep going // always
gr_hchar_prepright
	; Done left but prepare to do right
	; X=8 which is the right side of the image cache
	lda tmp_a			; Check if anything on the right
	beq gr_hchar_imagedone	; If not, then don't do it!
	...
	adc #8
	sta tmp_b
	bcc gr_hchar_getimage	; Only do right if in bounds
gr_hchar_imagedone
...
gr_box
	...
	sta tmp_b+1
gr_box_8line
	lda tmp_b+1					; Move left 1 column
	sec
	sbc #8
	sta tmp_b+1
	cmp tmp_b
	beq gr_box_do_vert			; if columns same then done
	ldx tmp_b+1					; Do top from right to left
	ldy num_a+1
	lda #0xff
	jsr gr_box_plot
	ldx tmp_b+1					; Do top from right to left
...
gr_box_tiny_width
	...
	sta tmp_b					; This is the intra column mask
	ldx num_a					; Plot top
	ldy num_a+1
	jsr gr_box_plot
...
gr_box_plot
	...
	ora tmp_c					; Assume mode 1 (or)
	cpx #1						; Done if correct
	beq gr_box_write
	eor tmp_c					; Assume mode 0 (eor erases or)
	cpx #0						; Done if correct
	beq gr_box_write
	lda tmp_c+1					; Else eor with source
	eor tmp_c
gr_box_write
...
gr_scroll_read_ln
	jsr vdp_rd_vram
	sta scratch+128,x			; +128 to avoid string corruption, fingers crossed!
	dex
